...
package com.github.palindromicity.maven.plugins.bundles;
...
/**
 * Packages the current project as an Bundle Archive (BUNDLE).
 * Bundles are based on Apache Nifi Archives (NAR) 1.2.0
 *
...
public class BundleMojo extends AbstractMojo {
   ...
   */
  @Parameter(alias = "packageIDPrefix",
      property = "bundle.packageIDPrefix",
      defaultValue = "Bundle",
      required = true)
   ...
   *
   * For example if the pom.xml for the test-bundle contained the following:
   *
   ...
   *
   *    <properties>
   *       <bundleGroup>org.test.overridden</bundleGroup>
   *       <bundleId>overridden-test-bundle</bundleId>
   *       <bundleVersion>2.0</bundleVersion>
   *   </properties>
   *
   * It would produce a MANIFEST with:
   *
   *   Bundle-Id: overridden-test-bundle
   *   Bundle-Group: org.test.overridden
   *   Bundle-Version: 2.0
   *
   */
  ...
  protected void copyArtifact(Artifact artifact) throws MojoExecutionException {
    ...
    final File destDir = DependencyUtil
        .getFormattedOutputDirectory(false, false, false, false, false, getDependenciesDirectory(), artifact);
    final File destFile = new File(destDir, destFileName);
    copyFile(artifact.getFile(), destFile);
  }

  protected Artifact getResolvedPomArtifact(Artifact artifact) {
    Artifact pomArtifact = this.factory
  ...
  protected ArtifactsFilter getMarkedArtifactFilter() {
    return new DestFileFilter(this.overWriteReleases, this.overWriteSnapshots, this.overWriteIfNewer, false, false,
        false, false, false, getDependenciesDirectory());
  }
  ...
  protected DependencyStatusSets getClassifierTranslatedDependencies(Set artifacts, boolean stopOnFailure)
    ...
    if (StringUtils.isNotEmpty(copyDepClassifier)) {
      ArtifactTranslator translator = new ClassifierTypeTranslator(this.copyDepClassifier, this.packageType,
          this.factory);
  ...
  protected void copyFile(File artifact, File destFile) throws MojoExecutionException {
    try {
      getLog().info(
          "Copying " + (this.outputAbsoluteArtifactFilename ? artifact.getAbsolutePath() : artifact.getName()) + " to "
              + destFile);
  ...
  public File createArchive() throws MojoExecutionException {
    ...
    try {
      ...
      if (bundleDependency != null) {
        final String bundleDependencyGroup =
            notEmpty(this.bundleDependencyGroup) ? this.bundleDependencyGroup : bundleDependency.getGroupId();
        final String bundleDependencyId =
            notEmpty(this.bundleDependencyId) ? this.bundleDependencyId : bundleDependency.getArtifactId();
        final String bundleDependencyVersion =
            notEmpty(this.bundleDependencyVersion) ? this.bundleDependencyVersion : bundleDependency.getVersion();
      ...
      return bundleFile;
    } catch (ArchiverException | MojoExecutionException | ManifestException
        | IOException | DependencyResolutionRequiredException e) {
  ...
  private BundleDependency getBundleDependency() throws MojoExecutionException {
    ...
    if (artifacts.size() > 1) {
      throw new MojoExecutionException(
          "Each Bundle represents a ClassLoader. A Bundle dependency allows that Bundle's ClassLoader to be "
              + "used as the parent of this Bundle's ClassLoader. As a result, only a single Bundle dependency is "
              + "allowed.");
    } else if (artifacts.size() == 1) {
      final Artifact artifact = (Artifact) artifacts.iterator().next();

      bundleDependency = new BundleDependency(artifact.getGroupId(),
